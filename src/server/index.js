    ...
    try {
      axios
        ...
        .catch(function (error) {
          console.log(
            "getOkapiToken: error getting okapi token"
          );
          console.log(error);
        });
    } catch (e) {
...
app.get("/api/getUser/:usr&:pass", (req, res) => {
  ...
  try {
    ...
    var authUrl =
      config["demo"]["folio_url"] + "authn/login";
    const authHeaders = {
      "Content-Type": "application/json",
      "x-okapi-tenant": config["demo"]["folio_tenandid"],
    };
            ...
            axios({
              ...
              .then(function () {
                axios({
                  ...
                  .then(function (response) {
                    recid =
                      response.data.holdingsRecords[0][
                        "id"
                      ];
                  })
                  .then(function () {
                ...
                axios({
                  ...
                  .then(function (resp) {
                    body = {
                      ...
                      item: {
                        status:
                          resp.data.items[0]["status"][
                            "name"
                          ],
                        title: resp.data.items[0]["title"],
                        barcode:
                          resp.data.items[0]["barcode"],
                        callNumber:
                          resp.data.items[0][
                            "itemLevelCallNumber"
                          ],
...
app.listen(process.env.PORT || 8081, () =>
  console.log(
    `Listening on port ${process.env.PORT || 8081}!`
  )
);
